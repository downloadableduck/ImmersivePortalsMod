...
import static com.mojang.blaze3d.shaders.ShaderType.VERTEX;

public class ShaderCodeTransformation {
    private static final Logger LOGGER = LogUtils.getLogger();
    
    public static enum PortalShaderType {
        vs, fs
    }
    
    private static boolean matches(com.mojang.blaze3d.shaders.ShaderType type, PortalShaderType me) {
        if (type == ShaderType.FRAGMENT) {
            return me == PortalShaderType.fs;
        }
        else if (type == ShaderType.VERTEX) {
            return me == PortalShaderType.vs;
        }
        return false;
    }
    ...
    private static Config getConfig(ShaderType type, String shaderId) {
        return configs.stream().filter(
            config -> matches(type, config.type) &&
