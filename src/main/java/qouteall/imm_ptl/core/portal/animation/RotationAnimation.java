...
public class RotationAnimation implements PortalAnimationDriver {
    ...
    private static RotationAnimation deserialize(CompoundTag tag) {
        ...
        double degreesPerTick = tag.getDouble("degreesPerTick").get();
        long startGameTime = tag.getLong("startGameTime").get();
        long endGameTime = tag.getLong("endGameTime").get();
        TimingFunction timingFunction = tag.contains("timingFunction") ?
            TimingFunction.fromString(tag.getString("timingFunction").get()) : null;
